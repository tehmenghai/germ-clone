"""
llm_clean_chunk_BEN0602.py

LLM-powered cleaning and concept-based chunking for the BEN0602 pipeline.

Replaces the two-step BEN0601 approach (regex clean → fixed sliding-window chunk) with
Gemini calls that implement AGENT_SKILL_BEN0602.md:
...
Input:  data_BEN0601/marker_output_BEN0601/**/*.md  (Marker-extracted Markdown)
Output: data_BEN0602/chunks_BEN0602/chunks_BEN0602.jsonl

Usage:
    python source/backend/ingestion/llm_clean_chunk_BEN0602.py
"""
...
# Reuse BEN0601 env file — same GEMINI_API_KEY and DATABASE_URL
ENV_FILE = PROJECT_ROOT / "source/backend/ingestion/.env_BEN0601"

# Input: Marker output produced by BEN0601 pipeline (already extracted)
MARKER_DIR = PROJECT_ROOT / "data_BEN0601/marker_output_BEN0601"

# Output: new BEN0602 enhanced chunks
CHUNKS_DIR = PROJECT_ROOT / "data_BEN0602/chunks_BEN0602"

# Agent skill prompt lives in Ben's sandbox docs
SKILL_FILE = PROJECT_ROOT / "sandbox/ben/docs_BEN0602/AGENT_SKILL_BEN0602.md"
...
def load_system_prompt() -> str:
    """
    Load AGENT_SKILL_BEN0602.md and append the array-output override.
    """
...
def load_existing_chunks(path: Path) -> tuple[dict[str, list[dict]], list[dict]]:
    """
    Read chunks_BEN0602.jsonl if it exists.
...
def main() -> None:
    if not SKILL_FILE.exists():
        print(f"[ERROR] AGENT_SKILL_BEN0602.md not found at {SKILL_FILE}")
        sys.exit(1)
    ...
    if not md_files:
        ...
        print("        Run extract_with_marker_BEN0601.py first.")
        sys.exit(1)

    # Load existing chunks so we can skip already-processed source files.
    full_path = CHUNKS_DIR / "chunks_BEN0602.jsonl"
    ...
    if not new_chunks and not existing_chunks:
        print("[ERROR] No chunks produced — check Gemini response and AGENT_SKILL_BEN0602.md")
        sys.exit(1)

    if not new_chunks:
        print(f"\n[INFO]  No new files to process — all {skipped} file(s) already chunked.")
        print("        To force reprocessing, delete the relevant rows from chunks_BEN0602.jsonl")
    ...
    sample_path = CHUNKS_DIR / "sample_chunks_BEN0602.jsonl"
    write_jsonl(sample_path, all_chunks[:5])
    print(f"[OK]    {sample_path.relative_to(PROJECT_ROOT)}  (first 5 — inspect these)")

    print(f"\n[NEXT]  Run upsert_pgvector_BEN0602.py to embed and load into Neon.")
