"""
dev_retrieve.py — local chunk inspector and retrieval tester.
...
Usage (from project root):
    python source/backend/ingestion/dev_retrieve.py stats
    python source/backend/ingestion/dev_retrieve.py browse
    python source/backend/ingestion/dev_retrieve.py browse --mod 3.3 --limit 10
    python source/backend/ingestion/dev_retrieve.py search "What is overfitting?"
    python source/backend/ingestion/dev_retrieve.py search "How does gradient descent work?" --top-k 8
"""
...
async def cmd_canonical_search(query: str, top_k: int) -> None:
    """--canonical mode: embeds with Gemini, displays all LLM-chunk enrichment fields."""
    # Load Gemini key from source/backend/.env
    load_dotenv(SCRIPT_DIR.parent / ".env", override=False)
    if not os.getenv("GEMINI_API_KEY"):
        print("[ERROR] GEMINI_API_KEY not found in source/backend/.env")
        sys.exit(1)

    sys.path.insert(0, str(SCRIPT_DIR))
    from embed_google import embed_document  # noqa: PLC0415
