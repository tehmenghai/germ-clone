"""
llm_chunk_transcript.py

LLM-powered semantic chunking for lecture transcripts (transcript pipeline).

Reads:  data_03_transcripts/parsed_vtt_{module}.jsonl  (from parse_vtt.py)
Reads:  slide markdown file(s) for topic alignment context
Reads:  module AGENT_SKILL markdown for LLM instructions

Writes: data_03_transcripts/chunks_transcript_{module}.jsonl  (LLM-chunk-schema JSONL)

Differences from llm_clean_chunk.py:
...
Usage (from project root):
    python source/backend/ingestion/llm_chunk_transcript.py
    python source/backend/ingestion/llm_chunk_transcript.py --module 3.2a
    python source/backend/ingestion/llm_chunk_transcript.py --module 3.2a --sample
...
# ---------------------------------------------------------------------------
# Helpers reused from the slide LLM-chunking pipeline
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Gemini call (same pattern as the slide LLM-chunking pipeline)
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    # Validate inputs
    for path, label in [(parsed_vtt, "parsed VTT"), (skill_file, "AGENT_SKILL"), (ENV_FILE, ".env")]:
        if not path.exists():
    ...
    if all_chunks:
        ...
        print("        Then run: python source/backend/ingestion/seed_from_llm_chunks.py \\")
