"""
seed_from_llm_chunks.py

Seeds the canonical production schema (documents / chunks / embeddings) from an
LLM-semantic-chunked JSONL file using Gemini gemini-embedding-2 (768-dim).

Accepts any JSONL that follows the LLM-chunk schema — works for both the slide
chunks produced by llm_clean_chunk.py and the transcript chunks
produced by llm_chunk_transcript.py.
...
Usage (from project root):
    # Default — seed slide chunks:
    python source/backend/ingestion/seed_from_llm_chunks.py

    # Seed a specific JSONL (e.g. transcript chunks):
    python source/backend/ingestion/seed_from_llm_chunks.py \\
        data_03_transcripts/chunks_transcript.jsonl
"""
...
DEFAULT_CHUNKS_PATH = PROJECT_ROOT / "data_02_llm_chunks/chunks/chunks.jsonl"

sys.path.insert(0, str(SCRIPT_DIR))
from embed_google import embed_document  # noqa: E402
...
def chunk_uuid(chunk_id: str) -> uuid.UUID:
    """Deterministic UUID from the LLM-chunk text chunk_id — same input always → same UUID."""
...
def main() -> None:
    ...
    # psycopg needs a plain postgresql:// DSN; .env uses SQLAlchemy's postgresql+asyncpg://
    ...
    if ok < 5:
        ...
        print('\n[NEXT]  Run dev_retrieve.py search "<query>" to verify retrieval.')
