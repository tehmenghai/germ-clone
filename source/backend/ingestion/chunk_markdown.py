...
Splits cleaned Markdown files from data_01_raw/processed/ into
retrieval-ready chunks and writes:
  - data_01_raw/chunks/chunks.jsonl   (all chunks)
  - data_01_raw/chunks/sample_chunks.jsonl  (first 5, for team inspection)
...
Usage (from project root):
    python source/backend/ingestion/chunk_markdown.py
"""
...
def main() -> None:
    ...
    cleaned_files = sorted(processed_dir.glob("*_cleaned.md"))
    if not cleaned_files:
        print(f"[ERROR] No cleaned files found in {processed_dir.relative_to(PROJECT_ROOT)}")
        print("        Run clean_marker_output.py first.")
        sys.exit(1)
    ...
    for cleaned_path in cleaned_files:
        # Derive original PDF name: strip _cleaned suffix
        pdf_stem = cleaned_path.stem.replace("_cleaned", "")
