  ...
  return (
    <div style={{ fontFamily: "var(--font-mono, monospace)" }}>
      ...
      >
        ...
        {/* X axis labels */}
        <text x={PAD.left} y={H - PAD.bottom + 14} fontSize={11} fill="var(--txt-faint)">0</text>
        <text x={W - PAD.right} y={H - PAD.bottom + 14} textAnchor="end" fontSize={11} fill="var(--txt-faint)">1</text>
        <text x={PAD.left + IW / 2} y={H - 8} textAnchor="middle" fontSize={11} fill="var(--txt-dim)">
          λ (regularisation strength)
        </text>

        {/* Y axis label */}
        <text x={12} y={PAD.top + IH / 2} textAnchor="middle" fontSize={11} fill="var(--txt-dim)"
        ...
        <g transform={`translate(${PAD.left + IW - 70}, ${PAD.top + 6})`}>
          {["w₁", "w₂", "w₃"].map((label, i) => (
            <g key={i} transform={`translate(0, ${i * 18})`}>
              <line x1={0} y1={0} x2={16} y2={0} stroke={COEF_COLORS[i]} strokeWidth={2} />
              <text x={20} y={4} fontSize={11} fill="var(--txt-dim)">{label}</text>
            </g>
          ))}
        </g>
      </svg>
